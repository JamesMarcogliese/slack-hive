...
def set_team(event_data):
	...
	slack_client.api_call("chat.delete", channel=channel, ts=event_data["message_ts"])
	
	slack_client.api_call("chat.postMessage", channel=channel, text="You now share notes with the " + selected_team + " team!", attachments=None)
	return

def review_notes(event_data):
	
	#add replace_original to messages 
	# add limit on number of returns
